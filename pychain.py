...
def setup():
    ...
    genesis_record = Record(sender="Genesis", receiver="", amount=0)
    # Create the genesis block with the genesis record
    genesis_block = Block(record=genesis_record, creator_id=0, prev_hash="0", timestamp=datetime.datetime.utcnow().strftime("%H:%M:%S"), nonce=0)
...
if st.button("Add Block"):
    ...
    # Create a new Record instance with sender, receiver, and amount
    record = Record(sender=sender, receiver=receiver, amount=amount)

    # Create a new Block instance with the new record
    new_block = Block(
        record=record,
        creator_id=42, # appropriate creator_id 
